 ...
 *
 * @brief hardware initialization functions specific to building the BR 1-pole AFGF breaker firmware
 *
 * this file will be copy to sys/hardware_init.c whenever the br_1p_afgf make target is invoked
 *
 * @file br_1p_afgf_hardware_init.c
...
void gpio_init_component(void)
{
    ...
    /**/
    LL_GPIO_SetOutputPin(MICRO_IS_ACTIVE_GPIO_Port, MICRO_IS_ACTIVE_Pin);
    ...
    ****************************************************/
    ...
    /**/
    GPIO_InitStruct.Pin = MICRO_IS_ACTIVE_Pin;
    GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
    GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_LOW;
    GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
    GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
    LL_GPIO_Init(MICRO_IS_ACTIVE_GPIO_Port, &GPIO_InitStruct);
